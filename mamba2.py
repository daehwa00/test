...
# site-packages/mamba_ssm/modules/mamba2.py
...
class Mamba2(nn.Module):
    def __init__(
        ...
        if not self.ef_enabled and self.reset_dt != 5.0:
            raise ValueError("reset_dt is only meaningful when ef_enabled is True")

        # Order: [z, x, B, C, dt]
        d_in_proj = 2 * self.d_inner + 2 * self.ngroups * self.d_state + self.nheads
        if self.process_group is None:
            projection = MemoryRebalancer if self.mr_enabled else nn.Linear
    ...
    def forward(
        ...
        if self.ef_enabled and resets is not None:
            resets = resets.to(dtype=torch.bool)
            if resets.shape != zxbcdt.shape[:2]:
                raise ValueError(
                    "resets must have shape (batch, seqlen) matching the input sequence"
                )
            dt_start = zxbcdt.shape[-1] - self.nheads
            projected_dt = zxbcdt[..., dt_start:]
            effective_dt = torch.where(
                resets.unsqueeze(-1),
                torch.full_like(projected_dt, self.reset_dt),
                projected_dt,
            )
            zxbcdt = torch.cat((zxbcdt[..., :dt_start], effective_dt), dim=-1)
...
class MemoryRebalancer(nn.Linear):
    def __init__(self, d_in, d_out, bias: bool = True, **kwargs):
